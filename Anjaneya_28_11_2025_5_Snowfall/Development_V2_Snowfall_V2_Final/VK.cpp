...
int WINAPI WinMain(HINSTANCE hInstance, HINSTANCE hPrevInstance, LPSTR lpszCmdLine, int iCmdShow)
{
	...
	SetFocus(hwnd);
        ...
        // Game Loop
        // NOTE: Autorepeated key events can fill the Windows message queue fast enough that
        // a full "drain the queue" loop will monopolize the frame budget and prevent the
        // update/render pass from running. Limit the number of messages handled per frame
        // so update()/display() still run once per frame even if the queue keeps refilling
        // with input events. WM_QUIT is still honored immediately when encountered.
        const unsigned int MAX_MESSAGES_PER_FRAME = 128;

        while (bDone == FALSE)
        {
                unsigned int messagesProcessed = 0;

                while (messagesProcessed < MAX_MESSAGES_PER_FRAME && PeekMessage(&msg, NULL, 0, 0, PM_REMOVE))
                {
                        messagesProcessed++;
...
VkResult CreateImagesAndImageViews(void)
{
	...
	*/
    ...
    VkDevice                                    device,
    VkImage                                     image, //whom to bind
    VkDeviceMemory                              memory, //what to bind
    VkDeviceSize                                memoryOffset);
        */
        vkResult = vkBindImageMemory(vkDevice, vkImage_depth, vkDeviceMemory_depth, 0); // We are binding device memory object handle with Vulkan image object handle.
        if (vkResult != VK_SUCCESS)
        {
                fprintf(gFILE, "CreateImagesAndImageViews(): vkBindImageMemory() function failed with error code %d\n", vkResult);
                return vkResult;
        }
        else
        {
                fprintf(gFILE, "CreateImagesAndImageViews(): vkBindImageMemory() succedded\n");
        }
