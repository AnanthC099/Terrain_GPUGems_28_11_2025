...
vec3 computeRuggedAlbedo(vec3 baseAlbedo, vec3 macroNormal, vec2 worldXZ, float heightSample)
{
    ...
    float snowBreakup = smoothstep(0.3, 0.7, fbm(worldXZ * 0.04));
    float snowCoverage = snowMask * slopeSnowAttenuation * snowBreakup;
    ...
    vec3 snowBlendedWithVegetation = mix(snow, soilTint, 0.25);
...
void main()
{
    ...
    vec3 terrainAlbedo = computeRuggedAlbedo(albedo, macroNormal, worldXZ, h);
    ...
    vec3 color = ambientColor * terrainAlbedo + diffuse + rimLight;
